...
summary_table <- df %>%                                        
  summarise(
    ...
    min   = min(loan_amnt, na.rm = T),                    
    q01   = quantile(loan_amnt,c(0.01) ),
    q02   = quantile(loan_amnt,c(0.02) ),
    q03   = quantile(loan_amnt,c(0.03) ),  
    q05   = quantile(loan_amnt,c(0.05) ),
    q10   = quantile(loan_amnt,c(0.10) ),
    q20   = quantile(loan_amnt,c(0.20) ),
               
    mean  = round(mean(loan_amnt, na.rm=T), digits = 1),  
    median  = round(median(loan_amnt, na.rm=T), digits = 1),  
    
    q80   = quantile(loan_amnt,c(0.80) ), 
    q90   = quantile(loan_amnt,c(0.90) ), 
    q95   = quantile(loan_amnt,c(0.95) ), 

    q98   = quantile(loan_amnt,c(0.97) ),
    q98   = quantile(loan_amnt,c(0.98) ),
    q99   = quantile(loan_amnt,c(0.99) ),  
    
    max   = max(loan_amnt, na.rm = T),                    
    sd    = round(sd(loan_amnt, na.rm = T), digits = 1),  
    missing         = sum(is.na(person_age),digits = 1)-1,
    pct_missing = scales::percent(missing / n()),
    valor_0     = sum(loan_amnt = 0, na.rm = T),               
...
```{r}
aa <-function(dataframe,variable) {
  
  tabla2 <- dataframe %>%                                        
  summarise(
    sd    = round(sd(variable, na.rm = T), digits = 1),  
