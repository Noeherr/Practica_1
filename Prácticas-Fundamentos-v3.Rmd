...
Generamos dos dataframe, uno para entrenamiento (train) y otro para validación (validation)
...
Scatter plot entre la variable dependiente, target y las independientes numéricas para ver relaciones
...
```{r}
algoritmo_imputacion = mice(train, m = 5, method = "pmm", seed = 300, printFlag = FALSE)
train <- complete(algoritmo_imputacion)
validacion <- complete(algoritmo_imputacion)
```
```{r}
dim(train)
dim(validacion)
...
Vemos de nuevo las relaciones entre el target y as variables numéricas una vez transformadas:
