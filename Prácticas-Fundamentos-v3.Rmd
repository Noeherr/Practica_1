...
**FALTAN CRUCES DE VARIABLES NUMÉRICAS (SCATTERPLOT) Y ALGÚN CRUCE MÁS**

Sccater plot entre la variable dependiente, target y las independientes numéricas para ver relaciones

```{r}

plot( train$target, train$person_age )
plot( train$target, train$person_income )
plot( train$target, train$person_emp_length )
plot( train$target, train$loan_int_rate )


#Use loess (local fiting as smooth method)
ggplot2.scatterplot(data=train, xName='wt',yName='mpg',
...
```{r,warning=FALSE}
#square
corrplot(corr_matrix, method = "pearson", shade.col = NA, tl.col = "black",
...
Primero haremos una regresión simple con la variable de mayor correlación con la variable dependinte y la interpretación de coficientes.

```{r}
lm_fit <- lm(target~person_income, data=train)
summary(lm_fit)
...
La ventaja de R2 es que es independiente de la escala en la que se mida la variable respuesta, por lo que su interpretación es más sencilla.

Los dos coeficientes de regresión (β0 y β1) estimados por el modelo son significativos y se pueden interpretar como:

Intercept(β0): El valor promedio del precio de la vivienda cuando el lstatus es 0 es de 34.5538 unidades.
Predictor lstat(β1): por cada unidad que se incrementa el predictor lstat el precio de la vivienda disminuye en promedio 0.9500 unidades.
...
Para este apartado nos basaremos fielmente en el trabajo elaborado por Victo Aceña en noviembre de 2021.
...
Una vez se haya seleccionado uno de los modelos, se pueden obtener los coeficientes para cada una de las variables con la instrucicción coef.
...
#plot(reg_sum$Cp ,xlab ="Number of Variables",ylab ="BIC", type ="l")
```
...
Tiene un R cuadradro ajustado de 0.1688 (16,88%)
...
ggplot(data = train, aes(x = predict(modelo_final), 
                        y = abs(rstudent(modelo_final))))+
geom_hline(yintercept = 3, color = "grey", linetype = "dashed")+
# se identifican en rojo las observaciones con residuos estandarizados absolutos > 3
geom_point(aes(color = ifelse(abs(rstudent(modelo_final)) > 3, "red", "black")))+
